...
export const copy = {
  en: {
    ...
    map: {
      ...
      high: 'High',
      watch: 'Watch',
      legendNote: 'H3 detection envelopes',
      detections: 'detections',
      summedFrp: 'summed FRP',
    },
    methodology: {
      ...
      collection: "EUMETSAT's Meteosat Third Generation (MTG) satellite, using its Flexible Combined Imager (FCI), supplies the fast 10-minute geostationary detections for this region through the public Tabula Caloris compatibility bridge. The website checks the live feed every 10 minutes. NASA FIRMS remains enabled for VIIRS and MODIS context when a free map key is configured.",
      ...
      geometry: 'The colored envelope is the union of H3 resolution-9 cells containing observations. It is not a fire perimeter, ignition location, burned-area product or spread forecast.',
    ...
    disclaimer: {
      ...
      arabicLabel: 'العربية',
      english: "This map shows satellite thermal anomaly detections from EUMETSAT's Meteosat Third Generation (MTG) satellite, using the MTG-FCI product delivered through LSA SAF, plus NASA FIRMS. A cluster means satellites detected heat in nearby pixels; it is not a confirmed fire perimeter, evacuation notice, or official emergency report. Clouds, smoke, terrain, sensor limits, and processing delays can hide or delay detections. Verify with local authorities and ground reports before acting.",
  ...
  ar: {
    ...
    methodology: {
      ...
      collection: 'يوفّر قمر الجيل الثالث من ميتيوسات (MTG) التابع لـ EUMETSAT، باستخدام جهاز التصوير المركّب المرن (FCI)، الأرصاد الحرارية السريعة من المدار الثابت بالنسبة إلى الأرض كل ١٠ دقائق لهذه المنطقة عبر جسر التوافق العام Tabula Caloris. ويتحقق الموقع من المصدر المباشر كل ١٠ دقائق. وتبقى NASA FIRMS مفعّلة لتوفير سياق VIIRS وMODIS عند ضبط مفتاح خريطة مجاني.',
      ...
      geometry: 'النطاق الملون هو اتحاد خلايا H3 بدقة 9 التي تحتوي على أرصاد. هذا ليس محيط حريق ولا نقطة اشتعال ولا منتج مساحة محترقة ولا توقع انتشار.',
...
const sourceLabelsAr: Record<string, string> = {
  ...
  MTG_FCI_LSA_SAF: 'MTG-FCI التابع لـ EUMETSAT عبر LSA SAF / Tabula Caloris',
  IMPORTED: 'ملف CSV مستورد',
};
